    ...
    def trackMouse(self, mouse_event):
        ...
        if self.camera.DepthFrameRaw.any() != 0:
            ...
            self.ui.rdoutMousePixels.setText("(%.0f,%.0f,%.0f)" %
                                             (pt.x(), pt.y(), z))
            pixels = np.array([[pt.x()],
                             [pt.y()],
                             [1]])
            C = z * np.matmul(np.linalg.inv(K), pixels)
            C_modified =  np.vstack([C, [1]])
            W = np.matmul(np.linalg.inv(H), C_modified)
            '''
            world_x = 0.001096*z*pt.x() + 0.716165*z
            world_y = -0.001095*z*pt.y() + 0.381139*z + 400
            world_z = 967.58 - z
            '''
            world_x = W[0]
            world_y = W[1]
